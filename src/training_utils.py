...
def record_episode_statistics(agent, done, truncated, info, num_envs):
    """
    Record episode statistics to tensorboard when episodes complete.
    
    Args:
        agent: The RL agent with summary writer
    ...
    for i in range(num_envs):
        if done[i] or truncated[i]:
            agent.summary_writer.add_scalar("Episode/Reward",
                                           info['accumulated_reward'][i])
            agent.summary_writer.add_scalar("Episode/Steps",
                                           info['episode_steps'][i])
...
def setup_training_environment(config, restart_training):
    """
