...
try:
    while True:
        ...
        state_to_record = np.array(current_state, dtype=np.uint8) # Ensure state is storable
        ...
        current_episode_traces.append((state_to_record, action, float(reward), next_state_to_record, bool(done), info))

        if done or end_episode or is_quitting:
            episode_count += 1
            print(f"Episode {episode_count} finished. Score: {info.get('score', 0)}. Traces in episode: {len(current_episode_traces)}")
            ...
            try:
                current_state = env.reset()
                if isinstance(current_state, tuple) and len(current_state) == 2: # For newer gym
                    current_state, info = current_state
    ...
    if recorded_traces:
        ...
        try:
            ...
            print(f"Total (state, action, reward, next_state, done) tuples: {len(recorded_traces)}")
        except Exception as e:
